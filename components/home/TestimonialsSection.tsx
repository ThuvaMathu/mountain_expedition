...
export function TestimonialsCarousel() {
  ...
  // Fetch approved experiences from Firestore
  useEffect(() => {
        ...
        if (!snapshot.empty) {
          const fetchedData = snapshot.docs.map(doc => {
            ...
            return {
              ...
              date: data.createdAt ? new Date(data.createdAt.seconds * 1000).toLocaleDateString(undefined, { day: 'numeric', month: 'short' }) : "Recently",
                  ...
                  <div className="bg-white rounded-2xl p-8 shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-gray-100 h-full flex flex-col justify-between hover:shadow-[0_8px_30px_rgb(0,0,0,0.08)] transition-all duration-300">
                    ...
                    <div className="flex items-center justify-between pt-6 border-t border-gray-50">
                      <div className="flex items-center gap-1">
                        <span className="font-bold text-gray-900 mr-2">{typeof item.rating === 'number' ? item.rating.toFixed(1) : item.rating}</span>
                        {[...Array(5)].map((_, i) => (
                          <Star key={i} className={cn("w-4 h-4", i < (item.rating || 5) ? "text-orange-400 fill-current" : "text-gray-200")} />
                        ))}
                      </div>
