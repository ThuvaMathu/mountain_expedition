...
export const PhoneInput: React.FC<PhoneInputProps> = ({
  ...
  const handleCountrySelect = (country: CountryCode) => {
    ...
    // Remove leading 0 if present (for international format consistency)
    let sanitizedPhone = phoneNumber.trim();
  ...
  const handlePhoneChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    let sanitized = inputValue.replace(/[^\d\s]/g, "");

    // Collapse multiple spaces into single space and trim
    sanitized = sanitized.replace(/\s+/g, " ").trim();

    // Remove leading 0 for international format (common in many countries)
    // When using international dial code, the leading 0 should be removed
      ...
      <div className="flex gap-1.5 w-full">
        ...
        <div className="relative flex-shrink-0" ref={dropdownRef}>
          <button
            ...
            className={`flex items-center gap-1 px-2 py-2 border border-gray-300 rounded-md bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-teal-500 h-[42px] ${
          ...
          {isOpen && (
            <div className="absolute z-50 mt-1 w-72 sm:w-80 bg-white border border-gray-300 rounded-md shadow-lg max-h-72 overflow-hidden left-0 sm:left-auto">
              {/* Search Input */}
              <div className="p-2 border-b sticky top-0 bg-white">
                <input
                  ...
                  className="w-full px-2 py-1.5 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-teal-500 text-sm"
                  autoFocus
                />
              </div>
