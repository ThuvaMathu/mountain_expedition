...
const RichTextEditor = dynamic(() => import("./rich-text-editor"), { ssr: false }) as any
...
export function BlogManagement() {
  ...
  const save = async () => {
    ...
    try {
      ...
      if (!isFirebaseConfigured || !db) {
        ...
        if (editingId) {
          setPosts((prev) => prev.map((p) => (p.id === editingId ? { ...payload, id: editingId } : p)))
        } else {
          setPosts((prev) => [{ ...payload, id: `demo_${Date.now()}` }, ...prev])
        }
        reset()
        return
      }
  ...
  const onFileChange: React.ChangeEventHandler<HTMLInputElement> = async (e) => {
    ...
    try {
      setUploading(true)
      setUploadProgress(0)
      const key = `posts/main-images/${Date.now()}_${file.name.replace(/\s+/g, "_")}`
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-gray-600">
          Create, edit, and delete blog posts. Upload a main image to feature on the blog.
        </p>
      </div>
      ...
      <div className="bg-white rounded-xl shadow p-6 space-y-5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div className="md:col-span-2">
            ...
            <Input value={tagsInput} onChange={(e) => setTagsInput(e.target.value)} placeholder="Everest, Training" />
          </div>
        </div>
        ...
        <div>
          ...
          <div className="flex flex-col md:flex-row gap-4 items-start">
            <div className="relative w-full md:w-72 aspect-[16/10] overflow-hidden rounded-lg border bg-gray-50">
              {form.mainImageUrl ? (
                ...
                <img src={form.mainImageUrl || "/placeholder.svg"} alt="Main" className="h-full w-full object-cover" />
              ) : (
            ...
            <div className="flex gap-2">
              ...
              <Button type="button" onClick={handleSelectFile} className="bg-teal-600 hover:bg-teal-700">
                <Upload className="h-4 w-4 mr-2" /> {form.mainImageUrl ? "Replace Image" : "Upload Image"}
              </Button>
              {form.mainImageUrl ? (
                <Button type="button" variant="outline" onClick={removeImage} className="bg-transparent">
          ...
          <p className="mt-2 text-xs text-gray-500">
            Recommended aspect ratio 16:10 or 16:9. Images are stored in Firebase Storage when configured.
          </p>
        </div>
        ...
        <div className="flex gap-2">
          <Button onClick={save} disabled={loading} className="bg-teal-600 hover:bg-teal-700">
            <Save className="h-4 w-4 mr-2" /> {editingId ? "Update Post" : "Publish Post"}
          </Button>
          {editingId && (
      ...
      <div className="bg-white rounded-xl shadow p-6">
        ...
        {posts.length === 0 ? (
          ...
          <div className="space-y-4">
            {posts.map((p) => (
              <div key={p.id} className="flex flex-col md:flex-row gap-4 border rounded-lg p-4">
                <div className="relative w-full md:w-52 aspect-[16/10] overflow-hidden rounded bg-gray-100">
                  {/* eslint-disable-next-line @next/next/no-img-element */}
                  <img
                    src={p.mainImageUrl || "/placeholder.svg?height=200&width=320&query=blog card image"}
                    alt={p.title}
                ...
                <div className="flex-1">
                  <div className="flex items-center justify-between gap-4">
                    ...
                    <div className="flex gap-2">
                      <Button size="sm" variant="outline" onClick={() => edit(p)} className="bg-transparent">
