...
// Empty trekking package structure
const emptyTrekkingPackage: TMountainType = {
  type: "trekking",
  name: "",
  altitude: 0,
  location: "",
...
export default function MountainManagement() {
  ...
  const [form, setForm] = useState<TMountainType>(emptyTrekkingPackage);
  ...
  const load = async () => {
    ...
    try {
      ...
      console.error("Error loading trekking packages:", error);
      setNotice("Failed to load trekking packages.");
    }
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editingId) {
        // Update
        await updateDoc(doc(db, "mountains", editingId), payload);
    ...
    <div className="space-y-8">
      ...
      <div>
        <h1 className="text-2xl font-bold text-gray-900">
          Trekking Package Management
        </h1>
        <p className="text-gray-600">
          Create and manage trekking packages for both domestic and
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <Input
              ...
              placeholder={
                form.category === "domestic"
                  ? "e.g., Nanda Devi Base Camp Trek"
                  : "e.g., Mount Kilimanjaro Expedition"
              }
            />
          </div>
          <div>
        ...
        <div className="grid grid-cols-2 gap-4 mb-6">
          <div className="bg-gradient-to-r from-teal-50 to-blue-50 rounded-lg p-4 border border-teal-200">
            ...
            <p className="text-2xl font-bold text-teal-600">
              {
                mountains.filter((pkg) => (pkg as any).category === "domestic")
                  .length
              }
            </p>
          </div>
        ...
        {mountains.length > 0 ? (
          ...
          <div className="text-center py-12 text-gray-500">
            ...
            <p className="text-lg font-medium mb-2">
              No trekking packages created yet
            </p>
            <p className="text-sm">
              Use the form above to add your first trekking package.
            </p>
          </div>
        )}
      </div>
