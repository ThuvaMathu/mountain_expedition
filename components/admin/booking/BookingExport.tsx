...
export function BookingExport({
  ...
  // Filter mountains based on product type - get unique destinations from bookings
  ...
  const getFilteredBookingsForExport = () => {
    ...
    if (exportFilters.mountainFilter !== "all") {
      filtered = filtered.filter(
        (booking) => booking.booking.id === exportFilters.mountainFilter
      );
    }
  ...
  const exportBasicSummary = () => {
    ...
    if (exportFilters.includeContactInfo) {
      headers.splice(6, 0, "Phone", "Emergency Contact");
    }
  ...
  const exportMountainSummary = () => {
    ...
    filtered.forEach((booking) => {
      const mountainId = booking.booking.id;
    ...
    <div className="bg-white rounded-xl shadow-lg p-6">
      ...
      <div className="mt-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 text-xs text-gray-600">
        <div className="p-3 bg-blue-50 rounded">
          ...
          booking information including type, organizer details, dates, and
          amounts.
        </div>
        ...
        <div className="p-3 bg-purple-50 rounded">
          ...
          Statistics grouped by mountain and type including revenue, bookings,
          and participant counts.
        </div>
        <div className="p-3 bg-orange-50 rounded">
          <strong className="text-orange-700">Financial Report:</strong> Monthly
          revenue breakdown with trekking/tour split and booking statistics.
        </div>
      </div>
    </div>
  );
